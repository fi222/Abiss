...
translations = {
    ...
    'he': {
        'title': 'מקודד/מפענח מאובטח',
        'msg_input': 'הודעה / קלט מקודד:',
        'key_label': 'מפתח (מספרים מופרדים בפסיקים):',
        'noise_label': 'רעש (1–15, לשימוש בהצפנה בלבד):',
        'encode': 'הצפן',
        'decode': 'פענח',
...
def set_language(lang):
    ...
    msg_label.config(text=translations[lang]['msg_input'])
    key_label.config(text=translations[lang]['key_label'])
    noise_label.config(text=translations[lang]['noise_label'])
    output_label.config(text=translations[lang]['output_label'])
...
noise_label = tk.Label(root, text='')
